...
collect "stig_2404_stigrule_270658_remote_server" "${REMOTE_SERVER}" "audisp remote syslog/SIEM destination (SV-270658)"
echo ""
...
collect "stig_2404_stigrule_270751_chrony_server" "${CHRONY_SERVER}" "Authoritative NTP server (SV-270751)"
echo ""
...
collect_list "stig_2404_stigrule_270748_authorized_sudo_members" \
...
collect "stig_2404_sssd_domain"          "${SSSD_DOMAIN}"      "SSSD domain name (SV-270734/735/736)"
collect "stig_2404_sssd_ldap_uri"        "${SSSD_LDAP_URI}"    "LDAP server URI"
collect "stig_2404_sssd_ldap_search_base" "${SSSD_SEARCH_BASE}" "LDAP search base DN"
echo ""
...
if [[ -n "$DOD_CA_FILE" ]]; then
  collect "stig_2404_stigrule_270745_dod_ca_filename" "$DOD_CA_FILENAME" "DoD Root CA (SV-270745)"
  if $YAML_MODE; then
    echo "stig_2404_stigrule_270745_dod_ca_content: |"
    sed 's/^/  /' "$DOD_CA_FILE"
  else
    echo "# DoD CA content found at: ${DOD_CA_FILE}"
    echo "# Use: stig_2404_stigrule_270745_dod_ca_content: <contents of ${DOD_CA_FILE}>"
  fi
else
  warn "No DoD CA certificate found in /usr/local/share/ca-certificates or /etc/ssl/certs"
  collect "stig_2404_stigrule_270745_dod_ca_filename" "dod-root-ca.crt" "DoD Root CA (SV-270745) — NOT FOUND, set manually"
  collect "stig_2404_stigrule_270745_dod_ca_content" "" ""
fi
echo ""
...
collect "stig_2404_stigrule_270744_pro_token" "" "Ubuntu Pro token (SV-270744) — must be set from Canonical account"
echo ""
...
collect "stig_2404_stigrule_270675_grub_pbkdf2_hash" "${GRUB_HASH}" "GRUB password hash (SV-270675)"
echo ""
...
collect_list "stig_2404_stigrule_270747_crypttab_entries" \
...
collect "stig_2404_stigrule_270816_audit_log_mountpoint" "${AUDIT_DIR}" "Audit log directory (SV-270816)"
collect "stig_2404_stigrule_270816_min_free_kb" "${AUDIT_FREE_KB}" "Current free space in KB (informational)"
...
collect "stig_2404_stigrule_270817_audit_offload_destination" "${OFFLOAD_DEST}" "Weekly audit offload rsync target (SV-270817)"
echo ""
...
if $YAML_MODE; then
  ...
  if [[ ${#LIMITED[@]} -eq 0 ]]; then
    echo "stig_2404_stigrule_270754_limited_services: []"
  else
    echo "stig_2404_stigrule_270754_limited_services:"
  ...
  printf "%-55s = [%s]\n" "stig_2404_stigrule_270754_limited_services" "$(IFS=,; echo "${LIMITED[*]}")"
fi
echo ""
...
if $YAML_MODE; then
  ...
  if [[ ${#ALLOWED[@]} -eq 0 ]]; then
    echo "stig_2404_stigrule_270719_allowed_ufw_rules: []"
  else
    echo "stig_2404_stigrule_270719_allowed_ufw_rules:"
  ...
  printf "%-55s = [%s]\n" "stig_2404_stigrule_270719_allowed_ufw_rules" "$(IFS=,; echo "${ALLOWED[*]}")"
fi
echo ""
