...
class CallbackModule(CallbackBase):
    ...
    def _record_stig_result_payload(
        ...
        *,
        ...
        # # In check_mode (audit), only assert and not_applicable phases
        # # produce meaningful compliance signals. Declarative and remediate
        # # tasks report changed=true when the system doesn't match desired
        # # state, which is expected — but that signal duplicates what the
        # # assert already captures. Recording it would let worst-status-wins
        # # override a passing assert with a "failed" from the declarative task.
        # if check_mode and phase not in ("assert", "not_applicable"):
        #     self._debug_stig_event(
        #         "skip-check-mode-non-assert",
        #         task_name=task_name,
        #         rule_num=rule_num,
        #         phase=phase,
        ...
        if is_not_applicable_task and not failed and not skipped:
            ...
            status = "failed" if (check_mode and changed) else ("fixed" if changed else "pass")
        else:
    ...
    def _extract_rule_number(self, task_name: str) -> str | None:
        ...
        m = re.search(r"\b([A-Z]+-\d+-\d{4,})\b", text, re.IGNORECASE)
        if m:
            return m.group(1)
