...
try:
    ...
    _core_path = (
        Path(__file__).resolve().parents[3] / "core" / "plugins" / "module_utils"
    )
    _norm_path = _core_path / "normalization.py"
    _spec = importlib.util.spec_from_file_location(
        "internal_core_normalization_helpers", _norm_path
    )
    ...
    _date_spec = importlib.util.spec_from_file_location(
        "internal_core_date_utils", _date_path
    )
    ...
    _prim_spec = importlib.util.spec_from_file_location(
        "internal_core_reporting_primitives", _prim_path
    )
