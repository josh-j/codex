...
class ComputedFilterCondition(BaseModel):
    ...
    field: str
    expression: str
    cmp: Literal["gt", "lt", "gte", "lte", "eq", "ne"]
    threshold: float
...
# ---------------------------------------------------------------------------
# Report sections (discriminated union on `type`)
# ---------------------------------------------------------------------------
...
class AlertPanelSection(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
ReportSection = Annotated[
    Union[KeyValueSection, TableSection, AlertPanelSection],
...
class ReportSchema(BaseModel):
    ...
    def _cross_check_references(self) -> "ReportSchema":
        """Ensure all field references in alerts and sections exist in fields."""
        declared = set(self.fields.keys())
        errors: list[str] = []

        for rule in self.alerts:
            cond = rule.condition
            if hasattr(cond, "field") and cond.field not in declared:
            ...
            if rule.affected_items_field and rule.affected_items_field not in declared:
        ...
        for section in self.sections:
            if isinstance(section, KeyValueSection):
                for kv in section.fields:
                    if kv.field not in declared:
                        errors.append(f"section '{section.id}': key_value references undeclared field '{kv.field}'")
            elif isinstance(section, TableSection):
                if section.rows_field not in declared:
                    errors.append(f"section '{section.id}': rows_field references undeclared field '{section.rows_field}'")
