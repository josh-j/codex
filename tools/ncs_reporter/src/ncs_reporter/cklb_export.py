...
def generate_cklb(
    ...
    cklb = {
        ...
        "target_data": {
            ...
            "host_name": hostname,
            "fqdn": hostname,
            "ip_address": "", # TODO: Add IP if available
            "role": "Workstation",
    ...
    for stig in skeleton.get("stigs", []):
        ...
        for rule in stig.get("rules", []):
            ...
            audit_result = (
                audit_map.get(rule_version) or 
                audit_map.get(rule_id) or 
                audit_map.get(group_id)
            )
            ...
            if audit_result:
                ...
                if res_status in ["failed", "fail", "open"]:
                    status = "open"
                    raw_details = audit_result.get("checktext") or audit_result.get("description") or "Automated check identified a finding."
                    ...
                    raw_details = audit_result.get("checktext") or audit_result.get("description") or "Automated check verified compliance."
                    details = _strip_html(raw_details)
            ...
            new_stig["rules"].append(new_rule)
        
        cklb["stigs"].append(new_stig)
