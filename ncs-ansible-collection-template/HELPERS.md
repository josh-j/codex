...
# <<<
---
- name: "__COLLECTION_NAME__ | Rotate local account password"
  hosts: "{{ __COLLECTION_NAME___target_hosts | default('__COLLECTION_NAME___servers') }}"
  ...
```
...
1. **Build a tarball** in `ncs-ansible`:
   ```bash
   just build-collection __COLLECTION_NAME__
   ```
   This needs the sibling repo at `../ncs-ansible-__COLLECTION_NAME__/`.
...
3. **Add to `requirements.yml`** (Mode A block) so
   ...
   ```yaml
   - name: "./collections/vendor/internal-__COLLECTION_NAME__-0.1.0.tar.gz"
     type: file
   ```
...
4. **Install + verify**:
   ...
   ansible-galaxy collection list | grep __COLLECTION_NAME__
   ansible-playbook --syntax-check -i inventory/production internal.__COLLECTION_NAME__.<some_playbook>
   ```

5. **Commit the tarball + requirements.yml bump**.

From there, `just release-collection __COLLECTION_NAME__ <version> "<message>"`
