...
class CallbackModule(CallbackBase):
    ...
    def _host_name(self, result):
        # Allow tasks to override host attribution via stig_target_host variable.
        # This lets API-based platforms (VMware) attribute results to the correct
        # remote host even though tasks execute on localhost.
        try:
            override = result._task.vars.get("stig_target_host")
            if override:
                return str(override)
        except Exception:
            pass
