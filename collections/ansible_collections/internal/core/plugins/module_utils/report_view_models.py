    ...
    for hostname, bundle in _iter_hosts(aggregated_hosts):
        ...
        if stig:
            stig_rows.append(
                {
                    "name": hostname,
                    "health": _status_from_health(stig.get("health")),
                    "open_findings": len(list(stig.get("alerts") or [])),
                }
            )
    ...
    *,
    ...
    for hostname, bundle in _iter_hosts(aggregated_hosts):
        ...
        for audit_type, payload in dict(bundle or {}).items():
            if str(audit_type).lower().startswith("stig") and isinstance(payload, dict):
                stig_entries.append(
                    {
                        "host": hostname,
                        "audit_type": str(audit_type),
                        "health": _status_from_health(payload.get("health")),
