...
def build_audit_export_payload(
    ...
    IMPORTANT:
    The export schema requires vcenter_health.summary to always contain:
      - total
      - critical_count
      - warning_count
      - info_count
    ...
    data = {
        ...
        "vcenter_health": {
            ...
            "audit_failed": bool(audit_failed),
            "data": copy.deepcopy(
                (vmware_ctx.get("vcenter_health") or {}).get("data") or {}
            ),
        },
        "check_metadata": {
...
def audit_resource_rollup(clusters, cpu_crit=90, cpu_warn=80, mem_crit=90, mem_warn=80):
    ...
    mem_caps = [
        _to_int(((c or {}).get("utilization") or {}).get("mem_total_mb", 0))
        for c in clusters
    ]
    mem_useds = [
        _to_int(((c or {}).get("utilization") or {}).get("mem_used_mb", 0))
        for c in clusters
    ]
...
def attach_audit_results(vmware_ctx, alerts, audit_failed, health, summary):
    ...
    vmware_ctx.setdefault("vcenter_health", {})
    vmware_ctx["vcenter_health"]["health"] = health_val
    vmware_ctx["vcenter_health"]["summary"] = summary_dict
    vmware_ctx["vcenter_health"]["alerts"] = alerts_list
