# internal.vmware/plugins/module_utils/audit.py
# Pure business logic for VMware audit alert evaluation.
...
_as_list = _prim.as_list
_alert = _prim.build_alert
_build_count_alert = _prim.build_count_alert
_build_threshold_alert = _prim.build_threshold_alert
...
append_alerts = _alerts.append_alerts
...
def build_audit_export_payload(
    ...
    """
    Builds the audit export payload.

    IMPORTANT:
    The export schema requires vmware_vcenter.summary to always contain:
      - total
      - critical_count
      - warning_count
      - info_count
    ...
    This function now enforces that shape even if callers pass an empty/partial summary.
    """
    alerts = list(vmware_alerts or [])
    vmware_ctx = dict(vmware_ctx or {})
    thresholds = dict(thresholds or {})

    # health is a scalar rollup string in your pipeline (e.g. "HEALTHY"/"WARNING"/"CRITICAL")
    health_val = str(health or "UNKNOWN")

    # Enforce required summary keys (schema contract)
    summary_in = summary or {}
    ...
    summary_out.update(summary_in)

    # Ensure by_category remains a mapping
    if not isinstance(summary_out.get("by_category"), dict):
        summary_out["by_category"] = {}
    ...
    data = {
        ...
        "vmware_vcenter": {
            ...
            "audit_failed": bool(audit_failed),
            "data": copy.deepcopy(
                (vmware_ctx.get("vmware_vcenter") or {}).get("data") or {}
            ),
        },
        "check_metadata": {
...
def audit_health_alerts(vmware_ctx):
    ...
    dc_count = _to_int(
        ((vmware_ctx.get("summary") or {}).get("datacenter_count", 0)), 0
    )
...
def audit_cluster_configuration_alerts(clusters, cpu_threshold=90, memory_threshold=90):
    clusters = _as_list(clusters)
    cpu_threshold = _to_float(cpu_threshold, 90)
    memory_threshold = _to_float(memory_threshold, 90)
    cluster_alerts = []
    noncompliant = []
    ...
    for item in clusters:
        ...
        memory_pct = _to_float(util.get("memory_pct", 0), 0)
        ...
        if memory_pct > memory_threshold:
            cluster_alerts.append(
                _alert(
                    "WARNING",
                    "cluster_capacity",
                    f"Memory Saturation: Cluster '{name}' is at {memory_pct}%",
                    {
                        "cluster": name,
                        "current_pct": memory_pct,
    ...
    if noncompliant:
        rollup_alerts.append(
            _alert(
                ...
                {},
                affected_items=[
                    (c or {}).get("name") for c in noncompliant if (c or {}).get("name")
                ],
            )
        )
...
def audit_snapshot_alerts(
    vmware_ctx, age_warning_days=7, size_warning_gb=100, max_items=25
):
    vmware_ctx = dict(vmware_ctx or {})
    summary = dict(
        ((vmware_ctx.get("inventory") or {}).get("snapshots") or {}).get("summary")
        or {}
    )
...
def audit_tools_alerts(vmware_ctx, max_items=50):
    ...
    return [
        _alert(
            ...
            affected_items=[
                (vm or {}).get("name") for vm in unhealthy if (vm or {}).get("name")
            ][:max_items],
...
def audit_resource_rollup(clusters, cpu_crit=90, cpu_warn=80, memory_crit=90, memory_warn=80):
    clusters = _as_list(clusters)
    cpu_caps = [
        _to_int(((c or {}).get("utilization") or {}).get("cpu_total_mhz", 0))
        for c in clusters
    ]
    cpu_useds = [
        _to_int(((c or {}).get("utilization") or {}).get("cpu_used_mhz", 0))
        for c in clusters
    ]
    memory_caps = [
        _to_int(((c or {}).get("utilization") or {}).get("memory_total_mb", 0))
        for c in clusters
    ]
    memory_useds = [
        _to_int(((c or {}).get("utilization") or {}).get("memory_used_mb", 0))
        for c in clusters
    ]

    cpu_cap = sum(cpu_caps)
    cpu_used = sum(cpu_useds)
    memory_cap = sum(memory_caps)
    memory_used = sum(memory_useds)
    cpu_pct = round((cpu_used / max(cpu_cap, 1)) * 100.0, 1)
    memory_pct = round((memory_used / max(memory_cap, 1)) * 100.0, 1)
    ...
    memory_alert = _build_threshold_alert(
        memory_pct,
        memory_crit,
        memory_warn,
        "capacity",
        f"Memory Saturation: {memory_pct}%",
        detail={"memory_used_mb": memory_used, "memory_total_mb": memory_cap},
        value_key="usage_pct",
    )
...
def attach_audit_utilization(vmware_ctx, utilization):
    ...
    vmware_ctx.setdefault("vmware_vcenter", {})
    vmware_ctx["vmware_vcenter"].setdefault("data", {})
    vmware_ctx["vmware_vcenter"]["data"]["utilization"] = utilization
    return vmware_ctx
...
def attach_audit_results(vmware_ctx, alerts, audit_failed, health, summary):
    ...
    # Health: accept str (preferred), but tolerate dict inputs
    ...
    health_val = str(health_val or "UNKNOWN")

    # Summary: always a dict
    summary_dict = dict(summary or {})

    vmware_ctx["alerts"] = alerts_list
    vmware_ctx.setdefault("vmware_vcenter", {})
    vmware_ctx["vmware_vcenter"]["health"] = health_val
    vmware_ctx["vmware_vcenter"]["summary"] = summary_dict
    vmware_ctx["vmware_vcenter"]["alerts"] = alerts_list
    vmware_ctx["vmware_vcenter"]["audit_failed"] = bool(audit_failed)
    vmware_ctx["vmware_vcenter"].setdefault("data", {})
    return vmware_ctx
...
def build_owner_notification_context(vmware_ctx, owner_email):
    ...
    my_vms = [
        vm
        for vm in all_vms
        if isinstance(vm, dict) and str(vm.get("owner_email") or "") == owner_email
    ]
    ...
    def _by_name(items, key="name"):
        return [
            ...
            if isinstance(item, dict) and str(item.get(key) or "") in my_name_set
        ]

    powered_off = [
        vm for vm in my_vms if str(vm.get("power_state") or "").upper() == "POWEREDOFF"
    ]
