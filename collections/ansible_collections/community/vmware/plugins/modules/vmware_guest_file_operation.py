...
class VmwareGuestFileManager(PyVmomi):
    def __init__(self, module):
        ...
        if datacenter_name:
            ...
            if not datacenter:
                module.fail_json(msg="Unable to find {datacenter} datacenter".format(**module.params))

        cluster = None
        if cluster_name:
            cluster = find_cluster_by_name(self.content, cluster_name, datacenter)
            if not cluster:
                module.fail_json(msg="Unable to find {cluster} cluster".format(**module.params))
    ...
    def directory(self):
        ...
        if operation in ("create", "mktemp"):
            try:
                ...
                result['msg'] = f"Guest directory {path} already exist: {to_native(file_already_exists.msg)}"
            except vim.fault.GuestPermissionDenied as permission_denied:
                self.module.fail_json(msg=f"Permission denied for path {path} : {to_native(permission_denied.msg)}",
                                      uuid=self.vm.summary.config.uuid)
            except vim.fault.InvalidGuestLogin as invalid_guest_login:
                self.module.fail_json(msg=f"Invalid guest login for user {vm_username} : {to_native(invalid_guest_login.msg)}",
                                      uuid=self.vm.summary.config.uuid)
            # other exceptions
            except Exception as e:
                self.module.fail_json(msg=f"Failed to Create directory into VM VMware exception : {to_native(e)}",
                                      uuid=self.vm.summary.config.uuid)
        ...
        if operation == "delete":
            try:
                ...
                result['changed'] = False
                result['msg'] = f"Guest directory {path} not exists {to_native(file_not_found.msg)}"
            except vim.fault.FileFault as e:
                self.module.fail_json(msg=f"FileFault : {e.msg}",
                                      uuid=self.vm.summary.config.uuid)
            except vim.fault.GuestPermissionDenied as permission_denied:
                self.module.fail_json(msg=f"Permission denied for path {path} : {to_native(permission_denied.msg)}",
                                      uuid=self.vm.summary.config.uuid)
            ...
            # other exceptions
                ...
                self.module.fail_json(msg=f"Failed to Delete directory into Vm VMware exception : {to_native(e)}",
                                      uuid=self.vm.summary.config.uuid)
    ...
    def fetch(self):
        ...
        try:
            ...
            if info.get('status') != 200 or not resp:
                self.module.fail_json(msg="Failed to fetch file : {}".format(info.get('msg', '')), body=info.get('body', ''))
            try:
                with open(dest, "wb") as local_file:
                    local_file.write(resp.read())
            except Exception as e:
                self.module.fail_json(msg=f"local file write exception : {to_native(e)}",
                                      uuid=self.vm.summary.config.uuid)
        except vim.fault.FileNotFound as file_not_found:
            self.module.fail_json(msg=f"Guest file {src} does not exist : {to_native(file_not_found.msg)}",
                                  uuid=self.vm.summary.config.uuid)
        except vim.fault.FileFault as e:
            self.module.fail_json(msg=f"FileFault : {to_native(e.msg)}",
                                  uuid=self.vm.summary.config.uuid)
        except vim.fault.GuestPermissionDenied:
            self.module.fail_json(msg=f"Permission denied to fetch file {src}",
                                  uuid=self.vm.summary.config.uuid)
        except vim.fault.InvalidGuestLogin:
            self.module.fail_json(msg=f"Invalid guest login for user {vm_username}",
                                  uuid=self.vm.summary.config.uuid)
        # other exceptions
        except Exception as e:
            self.module.fail_json(msg=f"Failed to Fetch file from Vm VMware exception : {to_native(e)}",
                                  uuid=self.vm.summary.config.uuid)
    ...
    def copy(self):
        ...
        if not os.path.exists(b_src):
            self.module.fail_json(msg=f"Source {src} not found")
        if not os.access(b_src, os.R_OK):
            self.module.fail_json(msg=f"Source {src} not readable")
        if os.path.isdir(b_src):
            self.module.fail_json(msg=f"copy does not support copy of directory: {src}")
        ...
        try:
            url = file_manager.InitiateFileTransferToGuest(vm=self.vm, auth=creds, guestFilePath=dest,
                                                           ...
                                                           fileSize=file_size)
            url = url.replace("*", hostname)
            _resp, info = urls.fetch_url(self.module, url, data=data, method="PUT", timeout=self.timeout)

            status_code = info["status"]
            if status_code != 200:
                self.module.fail_json(msg=f'problem during file transfer, http message:{info}',
                                      uuid=self.vm.summary.config.uuid)
        except vim.fault.FileAlreadyExists:
            result['changed'] = False
            result['msg'] = f"Guest file {dest} already exists"
            return result
        except vim.fault.FileFault as e:
            self.module.fail_json(msg=f"FileFault:{to_native(e.msg)}",
                                  uuid=self.vm.summary.config.uuid)
            ...
            self.module.fail_json(msg="Permission denied to copy file into "
                                      f"destination {dest} : {to_native(permission_denied.msg)}",
                                  uuid=self.vm.summary.config.uuid)
        except vim.fault.InvalidGuestLogin as invalid_guest_login:
            self.module.fail_json(msg="Invalid guest login for user"
                                      f" {vm_username} : {to_native(invalid_guest_login.msg)}")
        # other exceptions
        except Exception as e:
            self.module.fail_json(msg=f"Failed to Copy file to Vm VMware exception : {to_native(e)}",
                                  uuid=self.vm.summary.config.uuid)
        return result
