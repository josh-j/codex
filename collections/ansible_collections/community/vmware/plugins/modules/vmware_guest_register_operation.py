...
class VMwareGuestRegisterOperation(PyVmomi):
    ...
    def execute(self):
        ...
        if not datacenter:
            self.module.fail_json(msg=f"Cannot find the specified Datacenter: {self.datacenter}")
        ...
        if (self.folder in [None, "", "/"]):
            ...
            fullpath = f"{dcpath}{self.datacenter}{self.folder}"
        else:
            fullpath = f"{dcpath}{self.folder}"

        folder_obj = self.content.searchIndex.FindByInventoryPath(inventoryPath=f"{fullpath}")
        if not folder_obj:
            details = {
        ...
        if self.state == "present":
            ...
            if self.cluster:
                ...
                if not cluster_obj:
                    self.module.fail_json(msg=f"Cannot find the specified cluster name: {self.cluster}")
                ...
                if not resource_pool_obj:
                    self.module.fail_json(msg=f"Cannot find the specified resource pool: {self.resource_pool}")
            else:
            ...
            changed = False
            try:
                changed, _info = wait_for_task(task)
